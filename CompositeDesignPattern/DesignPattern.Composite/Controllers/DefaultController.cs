...
namespace DesignPattern.Composite.Controllers
{
    public class DefaultController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            var values = Rekursive(categories, new Category { CategoryName = "FirstParentCategory", CategoryID = 0 },
                new ProductComposite(0, "FirstComposite"));
            ViewBag.v = values;
            return View();
        }
        public ProductComposite Rekursive(List<Category> categories, Category firstParentCategory, ProductComposite firstComposite, ProductComposite leaf=null )
        {
            categories.Where(x => x.ParentCategoryID == firstParentCategory.CategoryID).ToList().ForEach(y=>
            {
